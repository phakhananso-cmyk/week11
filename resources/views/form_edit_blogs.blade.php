...
<div class="container" style="max-width: 750px;">
    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-3 mb-5">
        <div>
            ...
            <p class="text-muted mb-0">ปรับปรุงข้อมูลเนื้อหาบทความของคุณ</p>
        </div>
        <a href="{{ route('blog') }}" class="btn-modern-secondary text-decoration-none d-inline-flex align-items-center gap-2">
            <span>⬅️</span> กลับหน้ารวมบทความ
        </a>
    </div>
    ...
    <div class="card-modern border-0 p-4 p-md-5">
        <form action="{{ route('blog.update', $blog->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" class="form-control form-control-lg @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $blog->title) }}" placeholder="กรอกหัวข้อบทความที่น่าสนใจ..." style="border-radius: 12px; font-size: 1rem; border-color: #cbd5e1; padding: 0.8rem 1rem;">
                @error('title')
            ...
            <div class="mb-4">
                ...
                <textarea class="form-control @error('content') is-invalid @enderror" id="content" name="content" rows="8" placeholder="แบ่งปันข้อมูล รายละเอียดเนื้อหาที่นี่..." style="border-radius: 12px; font-size: 1rem; border-color: #cbd5e1; padding: 0.8rem 1rem;">{{ old('content', $blog->content) }}</textarea>
                @error('content')
                    <div class="invalid-feedback mt-2" style="font-weight: 500;">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <select class="form-select form-select-lg @error('status') is-invalid @enderror" id="status" name="status" style="border-radius: 12px; font-size: 1rem; border-color: #cbd5e1; padding: 0.8rem 1rem;">
                    ...
                    <option value="0" {{ old('status', $blog->status) == '0' ? 'selected' : '' }}>🟡 บันทึกเป็นฉบับร่าง (Draft)</option>
                </select>
                @error('status')
                    <div class="invalid-feedback mt-2" style="font-weight: 500;">{{ $message }}</div>
                @enderror
            ...
            <div class="d-flex justify-content-end gap-3">
                <a href="{{ route('blog') }}" class="btn-modern-secondary text-decoration-none">ยกเลิก</a>
                <button type="submit" class="btn-modern-primary">💾 อัปเดตข้อมูล</button>
            </div>
        </form>
    </div>
</div>
